 ...
 * No automatic gate redirect on page load.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initMagazine() {
        try {
            ...
            if(targetId) {
                ...
                if(article) checkGateAndOpenArticle(article);
            }
        } catch(e) {
    ...
    function renderMagazineView() {
        if(!allArticles.length) { if(emptyStateEl) emptyStateEl.style.display = 'flex'; return; }
    ...
    categoryNavButtons.forEach(btn=>btn.addEventListener('click',e=>{
